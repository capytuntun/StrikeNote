...
### 關聯分析
新增 → 關聯分析，自由畫布的節點／連線圖，像簡化版的 React Flow：拖曳節點到任意位置、拖節點右側的圓點連到另一個節點，雙擊空白處新增節點，雙擊節點或連線可以改文字，選取後按 Delete 刪除，也支援 Ctrl+Z 復原。適合畫攻擊路徑、主機關聯這類不是樹狀結構的圖。也可以用 ` ```relmap ` 語法把一張圖嵌進任何一篇一般筆記裡，跟心智圖一樣點「全螢幕」進去編輯。
