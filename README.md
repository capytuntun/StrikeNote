...
### 直接編輯筆記
從「新增 → 直接編輯筆記」建立，或在一般筆記的右鍵／⋯ 選單選「改用直接編輯」。打開後整個畫面就是排版好的頁面，像 Notion／Medium 一樣點哪一段就直接改那一段。語法跟一般筆記完全一樣（`#` 標題、`-` 清單、```` ``` ```` 程式碼、`/` 指令、`[[` 筆記連結），內容也仍然是 Markdown，所以 PDF 匯出、搜尋、版本歷史、共同編輯都照常運作。段落最後連按兩次 Enter 開新段落，Esc 結束編輯；想看原始碼時切到上方的「分割」。
