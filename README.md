...
- **課程筆記可以直接放檔案**：在資料夾裡按「上傳檔案」（或把檔案拖進頁面），PPTX、PDF、影片或任何檔案都會跟筆記放在同一個資料夾，分成「筆記」「檔案」兩段。PDF、MP4／WebM 影片、音訊與圖片點開就能在站內看（影片可以拖時間軸），PPTX 之類瀏覽器開不了的會給下載。圖片檔在清單裡直接顯示縮圖，滑鼠移到縮圖上會浮出較大的預覽。大檔案會自動切塊上傳、有進度條，斷線會從中斷的那一塊接續；單檔上限由 `UPLOAD_MAX_BYTES` 決定（預設 1 GiB）。檔案可以改名、移動、丟垃圾桶；從垃圾桶永久刪除時檔案本體一起刪掉（除非別篇筆記還在用它）。
